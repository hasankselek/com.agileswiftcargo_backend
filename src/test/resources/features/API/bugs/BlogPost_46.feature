Feature[US_046]:As an administrator (admin) I want to be able to create a new blog record via an API connection.
  ...
  Scenario Outline [TC_4601]: admin Send a POST request to the "api/blog/add" endpoint with valid authorization and the correct data
  ...
  "New Blog Added".
    ...
    * The api user verifies that the "message" information in the response body is "New Blog Added".
    ...
    Scenario Outline[TC_4602]: Invalid Token Send a POST request to the api/blog/add endpoint with invalid authorization and correct data
  ...
  Scenario Outline[TC_4603]: admin The new blog record to be created from the API must be verified from the API.
