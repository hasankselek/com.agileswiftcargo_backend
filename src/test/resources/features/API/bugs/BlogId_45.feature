...
Feature:[US_045] As an administrator (admin), I want to be able to access the detailed information
                 of the blog with the specified id number via the API connection.

  Scenario:[4501] Admin sends a GET request with valid authorization information and correct data (id)
  ...
  Scenario Outline: [4504] admin Send a GET request to the "api/blog/{id}" endpoint with valid authorization and a non-existent id,
  ...
  Scenario Outline:[4505] Invalid Token Send a GET request to the "api/blog/{id}" endpoint with invalid authorization, verify
