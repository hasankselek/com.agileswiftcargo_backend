Feature: As an administrator (admin), I want to be able to delete Delivery man information with the specified id number via API connection.
  ...
  Scenario Outline: admin Send a DELETE request to the api/deliveryman/delete/{id} endpoint with valid authorization and correct id, verify
  ...
  the id in the path. Then, confirm the deletion by sending a GET request to api/hub/{id} to verify the hub has been removed.
    ...
    * BThe api user verifies that the data Deleted id information in the returned response body is the same as the id path parameter written in the endpoint.
  ...
  information in the response body is "Unauthenticated.".
    ...
    * The api user sets "api/deliveryman/delete/408" path parameters.
    # Api kullanicisi "api/hub/delete/{id}" path parametrelerini olusturur
