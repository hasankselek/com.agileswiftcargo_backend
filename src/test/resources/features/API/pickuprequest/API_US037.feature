Feature: As an administrator (admin), I want to be able to update the regular pickup information with the specified id number via API connection.
  ...
  Scenario: admin Send a DELETE request to the api/pickuprequest/regular/edit/{id} endpoint with valid authorization and correct id, verify
  ...
  the id in the path. Then, confirm the deletion by sending a GET request to api/hub/{id} to verify the hub has been removed.
    ...
    * The api user sets "api/pickuprequest/delete/144" path parameters.
